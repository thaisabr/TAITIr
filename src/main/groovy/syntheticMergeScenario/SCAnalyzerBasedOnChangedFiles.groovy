...
@Slf4j
class SCAnalyzerBasedOnChangedFiles {
    ...
    boolean filterControllerConflict

    SCAnalyzerBasedOnChangedFiles(String analyzedTasksCsv){
        pairs = []
    ...
    SCAnalyzerBasedOnChangedFiles(String analyzedTasksCsv, boolean multipleExecutions){
        pairs = []
    ...
    private configureTasks(){
        ...
        filterEmptyInterfaces()
        configureTasksWithDate()
    }

    private configureTasksWithDate(){
        tasksWithDate = analyzedTasks.collect{
    ...
    private filterEmptyInterfaces(){
        analyzedTasks = analyzedTasks.subList(13, analyzedTasks.size())
        analyzedTasks = analyzedTasks.findAll{
            it[Constants.TESTI_INDEX]!="[]" //&& it[Constants.TASKI_INDEX]!="[]"
        }
    }

    private importConflictInfo(){
        conflictResult = CsvUtil.read(conflictResultFile)
        if(outputCsv.endsWith("-controller.csv") && conflictResult.size()<=1){
            filterControllerConflict = true
            return
        }
    ...
    List<String> extractConflictInfo(MergeTask leftTask, MergeTask rightTask){
        ...
        if(conflictResult.empty){
            conflictingFiles = leftTask.changedFiles.intersect(rightTask.changedFiles)
            if(filterControllerConflict){
                conflictingFiles = conflictingFiles.findAll{ Util.isControllerFile(it) }
            }
